...
<head>
  ...
  <script type='text/javascript' src="{{asset('design/assets/js/custom5223.js')}}"></script>
  	@toastr_css
  @livewireStyles
  	<link rel="stylesheet" href="{{asset('design/parfait_integration/css/evenement.css')}}">
</head>

<body> @include('partials.site_menu')
	<header class="bg-white"> @livewire('head',['viewname' => 'Evénement']) </header>
	<div class="container_card"> @foreach ($events as $event)
		<div class="card_fond">
			<div class="container_photo_fond"> <img src="{{asset('storage/'.$event->image)}}" alt="{{$event->titre}}"> </div>
			<h2 class="titre_fond">{{$event->titre}}</h2>
			<p class="paragraphe_fond">{!! $event->descriptif !!}</p>
			<div class="container_bouton_learn"> <a href="#" class="learn_button bouton_africkup anul_lien" data-type="{{$event->type}}" data-toggle="modal" data-target="#@if ($event->type == 'organisation'){{"modal-dejeuner"}}@else{{"modal-dejeuner-intermediation"}}@endif">Learn more</a> </div>
		</div> @endforeach </div>
	<!-- Modal 1 -->
	<div class="modal fade" id="modal-dejeuner" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content">
				<div class="container">
					<div class="text"> Contactez nous</div> @include('services.form_affaires') </div>
			</div>
		</div>
	</div>
	<!-- Modal 2 -->
	<div class="modal fade" id="modal-dejeuner-intermediation" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content">
				<div class="container">
					<div class="text"> Contactez nous</div> @include('services.form_relation') </div>
			</div>
		</div>
	</div>
